<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-gray-900">Admin - Adauga categorie</h2>
                <p class="mt-1 text-sm text-gray-500">Creeaza o noua categorie.</p>
            </div>
            ...
            <div class="rounded-lg bg-white p-8 shadow-sm ring-1 ring-gray-200">
                <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">Salveaza</button>
                    </div>
                </form>
            </div>
        </div>

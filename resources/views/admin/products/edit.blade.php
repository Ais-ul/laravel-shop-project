<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-gray-900">Admin - Editeaza Produs</h2>
    </x-slot>
                ...
                <form class="space-y-5" action="{{ route('admin.products.update', $product->id) }}" method="POST">
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" name="name" value="{{ old('name', $product->name) }}">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Slug</label>
                            <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" name="slug" value="{{ old('slug', $product->slug) }}">
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Descriere</label>
                        <textarea class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" name="description" rows="5">{{ old('description', $product->description) }}</textarea>
                    </div>

                    <div class="grid gap-5 sm:grid-cols-3">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Pret</label>
                            <input class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="number" name="price" step="0.01" value="{{ old('price', $product->price) }}">
                        </div>
                        <div>
                    ...
                    <div class="flex flex-wrap gap-3">
                        <button type="submit" class="rounded-md bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-500">Actualizeaza</button>
